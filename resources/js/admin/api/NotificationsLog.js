...
export function getUrl(params={}, subUrl='') {
    let url = `/api/v1/notifications-log${subUrl}`;
    let q = [];    
    params.isChecked ? q.push('is_checked=1') : (params.isChecked !== undefined ? q.push('is_checked=0') : null);
...
export function fetchNotificationsList(params={}) {
...
export function checkAllNotifications(params={}) {
    const url = getUrl(params, '/check-all');
    return new Promise((resolve, reject) => {
        fetch(url, {
            method: 'POST',
            headers: getBaseHeaders(),            
...
export function updateNotification(id, params) {    
    const url = getUrl() + `/${id}`;
    const data = {            
        is_checked: params.is_checked ? 1 : 0,        
    };
    return new Promise((resolve, reject) => {
        fetch(url, {
            method: 'PUT',
